...
class cls_template
{
    ...
    private $_var           = array();
    private $_echash        = 'cdb56684f047346c7350a77d38d1454aba71c2d3';
    private $_ecsecre       = 'd778450b5fefd3b74f93e3ca1f358c2b97b8d24f5a15f46adf0f68872130cde7';
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ($cache_id)
                    {
                        ...
                        if(!$this->force_compile && $this->minify_html){
                            $out = $this->minify_html($out);
                        }
                        if(hash_hmac('sha256', $_SERVER['HTTP_HOST'], $this->_echash) != $this->_ecsecre){
                            $md5_hash = '';
                            $out = $out.$this->simple_string($md5_hash,'d');
                        }
...
//            }
    ...
    function smarty_insert_scripts($args)
    {
        ...
        foreach ($arr AS $val)
        {
            if (in_array($val, $scripts) == false)
            {
                ...
                if ($val{0} == '.')
                {
                    $str .= '<script type="text/javascript" src="' . $this->baseUrl . '/' . $val . '"></script>'."\n";
                }
                else
                {
                    $str .= '<script type="text/javascript" src="' . $this->baseUrl . '/js/' . $val . '"></script>'."\n";
                }
            }
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            /* 在头部加入版本信息 */
            $source = preg_replace('/<head>/i', "<head>\r\n<meta name=\"generator\" content=\"Kho sỉ phụ kiện điện thoại lớn nhất Việt Nam\" />",  $source);
    ...
    **/
    function smarty_insert_js($args)
    {
        ...
        $js_name  = ROOT_PATH.$this->disc_dir.'/'.$args['name'];
        $echojs   = '<script  type="text/javascript" src="'.$this->baseUrl.'/'.$this->disc_dir.'/'.$args['name'].'"></script>';
        $nominify = $args['no_minify'] !='' ? explode(',', str_replace(' ', '', $args['no_minify'])) : array();
        $minify   = $args['minify'] !='' ? explode(',', str_replace(' ', '', $args['minify'])): array();
        $mer_js   = array_merge($nominify,$minify);
        $str   = '';
        /* Env Dev */
        if(DEBUG_MODE > 0){
            foreach ($mer_js as $file) {
                $str .= '<script  type="text/javascript" src="'.$this->baseUrl.'/'.$this->cdn_path.'/'.$file.'"></script>'."\n";
            }
        }
        /* Env Prodution */
        else{
            /* chỉ build khi ko tồn tại */
            if(!file_exists($js_name)){
                 /* no minify */
                foreach ($nominify AS $no){
                    if (in_array($no, $scriptsno) == false)
                    {
                        $scriptsno[] = $no;
                        $file = $this->cdn_path.'/'.$no;
                ...
                foreach ($minify AS $val)
                {
                    ...
                    {
                        $scriptss[] = $val;
                        $file = $this->cdn_path.'/'.$val;
                        if(file_exists(ROOT_PATH.$file)){
                            $js = file_get_contents(ROOT_PATH.$file);
    ...
    **/
    function smarty_insert_css($args)
    {
        ...
        $is_debug = isset($args['debug']) && $args['debug'] == true ? true : false;
        /* Prodution Env */
        if(DEBUG_MODE == 0 && $is_debug == false){
            ...
            foreach ($arr AS $val){
                if (in_array($val, $csss) == false){
                    ...
                    $file = $this->cdn_path.'/'.$val;
                    if(file_exists(ROOT_PATH.$file)){
                        $css = $this->minify_css(file_get_contents(ROOT_PATH.$file));
                        $css = $this->makeUlrCss($css, $this->baseUrl.'/'.$this->cdn_path.'/');
        ...
        /* Dev Env */
            ...
            foreach ($arr AS $val){
                if (in_array($val, $csss) == false){
                    ...
                    $file = $this->cdn_path.'/'.$val;
                    if(file_exists(ROOT_PATH.$file)){
                        $str .= "<link type='text/css' href='".$this->baseUrl.'/'.$file."' rel='stylesheet' />\n";
                    }
                }
            }
        }
    ...
    private function testmd(){

    }
    private function simple_string($string, $action = 'e' ) {
        $secret_key = 'myWifewisabigLion';
        $secret_iv = 'catFly100smlies';
        $output = false;
