...
class cls_image
{
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        {
            return str_replace(ROOT_PATH.CDN_PATH.'/', '', $img_name);
        }
        else
        {
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='', $filename= '')
    {
        ...
        {
            return str_replace(ROOT_PATH.CDN_PATH.'/', '', str_replace('\\', '/', realpath($img)));
        }
        ...
        {
            return str_replace(ROOT_PATH.CDN_PATH.'/', '', $dir) . $filename;
        }
        else
        {
    ...
    function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65)
    {
        ...
        {
            return str_replace(ROOT_PATH.CDN_PATH.'/', '', str_replace('\\', '/', realpath($filename)));
        }
        ...
        if ($path)
        {
            return str_replace(ROOT_PATH.CDN_PATH.'/', '', str_replace('\\', '/', $path));
        }
        else
        {
            $this->error_msg = $GLOBALS['_LANG']['writting_failure'];
    ...
    /**
     * Lưu hình
     * @param $filename path to save img /path/dir/img.png
     * @param $compression chất lượng nén
     * @param $image_type IMAGETYPE_JPEG | IMAGETYPE_GIF | IMAGETYPE_PNG | IMAGETYPE_WEBP
     * @param $permissions chmod
     */
    function save($filename, $compression=99, $image_type='',$permissions=null) {

        $image_type = !empty($image_type)? $image_type : $this->image_type;
        /* Tách lấy tên hình, bỏ đuôi đi */
    ...
    /* save img with setting default */
    function output($image_type=IMAGETYPE_JPEG) {
        $image_type = !isset($image_type)? $image_type : $this->image_type;
        if( $image_type == IMAGETYPE_JPEG ) {
    ...
    /* resize theo chiều cao, rộng */
    function resize($width,$height,$forcesize='n') {
      /* Nếu file nhỏ hơn kích thước resize, thì ko resize */
        if ($forcesize == 'n') {
    ...
    function getFilename($filenames){
        return preg_replace('/\\.[^.\\s]{3,4}$/', '', $filenames);;
    }

    function convertWebp($filename,$compression=90){
        $this->load($filename);
        return $this->save($filename, $compression, IMAGETYPE_WEBP);
    }
