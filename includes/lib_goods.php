...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name,icon, parent_id,is_show ' .
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, icon, thumb, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    $show_home = $cats == 'home' ? ' AND g.is_home = 1 ' : '';
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ...
               " $show_home ORDER BY g.goods_id DESC";
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
                    if ($data['is_best'] == 1)
                    {
                        $goods_data['best'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['goods_id']);
                    }
                    if ($data['is_new'] == 1)
                    {
                        $goods_data['new'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['goods_id']);
                    }
                    if ($data['is_hot'] == 1)
                    {
                        $goods_data['hot'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['goods_id']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.seller_note, g.is_hot,g.goods_number,  g.goods_name_style, g.deal_price, g.partner_price, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        $sql .= " $show_home ORDER BY g.sort_order, g.last_update DESC";
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            {
                $goods[$idx]['promote_price'] = $goods[$idx]['discount'] = '';
            }
            ...
            //$goods[$idx]['is_number']        = $row['is_number'];
            ...
            if(file_exists(ROOT_PATH.CDN_PATH.'/'.$thumb_webp)){
                $goods[$idx]['thumb_webp'] = $thumb_webp;
            }else{
                $goods[$idx]['thumb_webp'] = '';
            }
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,  g.seller_note, g.goods_name_style,g.goods_number, g.deal_price, g.partner_price, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.seller_note,g.is_hot,g.goods_number, g.goods_name_style, g.deal_price,g.partner_price, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    ...
    switch ($type)
    {
        ...
        case 'promote':
            $time = time();
            $sql .= " AND g.is_promote = 1 AND g.promote_start_date <= '$time' AND g.promote_end_date >= '$time'";
            break;
    }
...
{
    $time = time();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id,b.is_hot, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
    ...
    if ($row)
    {
        ...
        $row['price_final']  =  $promote_price> 0 ?  $promote_price : $row['shop_price'];
        $row['price_final'] = round($row['price_final'],0);
        $row['promote_price_org'] =  $promote_price;
        $row['promote_price'] =  price_format($promote_price);

        $row['deal_price'] =  price_format($row['deal_price']);
        $row['partner_price'] =  price_format($row['partner_price']);
        ...
        if($row['time_gift'] == 1){
            if($time >= $row['gift_start_date'] && $time <= $row['gift_end_date']){
                $row['goods_gifts']   = nl2p($row['goods_gifts']);
            }else{
                $row['goods_gifts'] = '';
            }
        }else{
        ...
        $thumb_webp = convertExtension($row['goods_thumb'], 'webp');
        if(file_exists(ROOT_PATH.CDN_PATH.'/'.$thumb_webp)){
            $row['goods_thumb_webp'] = $thumb_webp;
        }else{
            $row['goods_thumb_webp'] = '';
        }

        /* webp cho pc */
        $goods_img_webp = convertExtension($row['goods_img'], 'webp');
        if(file_exists(ROOT_PATH.CDN_PATH.'/'.$goods_img_webp)){
            $row['goods_img_webp'] = $goods_img_webp;
        }else{
            $row['goods_img_webp'] = '';
        }
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value, g.attr_price, g.display_type, g.display_value " .
    ...
    foreach ($res AS $row)
    {
        ...
        elseif($row['attr_type'] == 4){
            $arr['extra'][$row['attr_id']]['value'] = $row['attr_value'];
            $arr['extra'][$row['attr_id']]['name']     = $row['attr_name'];
            $arr['extra'][$row['attr_id']]['id']     = $row['attr_id'];
        }
        elseif($row['attr_type'] == 1 || $row['attr_type'] == 2)
        {
            $full_price = $final_price+intval($row['attr_price']);

            $arr['spe'][$row['attr_id']]['display_type'] = $row['display_type'];
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
                                                        ...
                                                        'format_price' => price_format(abs($full_price), false),
...
function get_same_attribute_goods($attr)
{
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            /* 查找符合条件的商品 */
            $sql = 'SELECT g.goods_id, g.goods_name,g.is_hot,g.goods_number, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                        "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                    "WHERE a.attr_id = '$key' AND g.is_on_sale=1 AND a.attr_value = '".$GLOBALS['db']->escape_string($val['value'],$GLOBALS['db'])."' AND g.goods_id <> '$GLOBALS[goods_id]' " .
            ...
            foreach ($res AS $row)
            {
                ...
                $lnk[$key]['goods'][$row['goods_id']]['is_hot']    = $row['is_hot'];
                $lnk[$key]['goods'][$row['goods_id']]['goods_number']    = $row['goods_number'];
 ...
 * @return  array
 */
function get_goods_gallery($goods_id, $type = 1, $limit = null)
{
    $limit = isset($limit) && $limit > 0 ? $limit : $GLOBALS['_CFG']['goods_gallery_number'];
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc, goods_attr_id ' .
        ' FROM ' . $GLOBALS['ecs']->table('goods_gallery') .
        " WHERE goods_id = '$goods_id' AND type = $type ORDER BY sort ASC LIMIT " . $limit;
    ...
    foreach($row as $key => $gallery_img)
    {
        ...
        $img_url_webp = convertExtension($gallery_img['img_url'], 'webp');
        if(file_exists(ROOT_PATH.CDN_PATH.'/'.$img_url_webp)){
            $row[$key]['img_url_webp'] = $img_url_webp;
        }else{
            $row[$key]['img_url_webp'] = '';
        }

        $thumb_url_webp = convertExtension($gallery_img['thumb_url'], 'webp');
        if(file_exists(ROOT_PATH.CDN_PATH.'/'.$thumb_url_webp)){
            $row[$key]['thumb_url_webp'] = $thumb_url_webp;
        }else{
            $row[$key]['thumb_url_webp'] = '';
        }
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.seller_note, g.goods_name_style, g.deal_price, g.click_count, g.partner_price, g.market_price, g.is_new, g.is_best, g.is_hot,g.goods_number,g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                 " IFNULL(AVG(r.comment_rank),0) AS comment_rank,IF(r.comment_rank,count(*),0) AS  comment_count, ".
                ' g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_thumb_url, g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            ' LEFT JOIN  '. $GLOBALS['ecs']->table('comment') .' AS r '.
                'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
            'WHERE g.is_home = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ') '.
            ' GROUP BY g.goods_id ';
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['comment_rank'] = ceil($row['comment_rank']) == 0 ? 1 : ceil($row['comment_rank']);
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name, icon, custom_name, ads_home, ads_home_mobile FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
    $cat_info = $GLOBALS['db']->getRow($sql);
    $cat = array();
    $cat['url']  = build_uri('category', array('cid' => $cat_id), $cat_info['cat_name']);
    $cat['id']   = $cat_id;
    $cat['name']   = $cat_info['cat_name'];
    $cat['icon']   = $cat_info['icon'];
    $cat['custom_name']   = $cat_info['custom_name'];
    $cat['ads_home']   = $cat_info['ads_home'];
    $cat['ads_home_mobile']   = $cat_info['ads_home_mobile'];
...
{
    $sql_sub = "SELECT cat_id, cat_name, thumb, icon  from ".$GLOBALS['ecs']->table('category')." WHERE parent_id=".$pid." AND is_show = 1 AND show_in_home = 1 ORDER BY sort_order ASC, cat_id ASC";
    ...
    if($subres)
    {
        foreach ($subres as $sidx => $subrow)
        {
            ...
            $children[$sidx]['icon']     = $subrow['icon'];
            $children[$sidx]['thumb']    = $subrow['thumb'];
...
{
    $sql =  'SELECT g.goods_id, g.goods_name,g.is_hot,g.goods_number, g.deal_price, g.partner_price, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb,g.goods_number,  g.is_hot, g.goods_img, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ".
    ...
    {
        $comments = getRank($row['goods_id']);
        $arr[$temp_index]['comment_rank']= round($comments['comment_rank'],1);
        $arr[$temp_index]['comment_count'] = $comments['comment_count'];
        ...
        $arr[$temp_index]['is_hot']        = $row['is_hot'];//配件的名称
        $arr[$temp_index]['goods_number']        = $row['goods_number'];//配件的名称
        ...
        /**
         * Nếu giá ưu đãi < giá khuyến mãi => Nếu giá ưu đãi
         * Nếu giá ưu đãi > giá khuyến mãi => giá khuyến mãi
         */

        if ($row['promote_price'] > 0){
            $promote_price = bargain_price($row['promote_price'], $row['promote_start_date'], $row['promote_end_date']);
            $promote_price = $promote_price > $row['goods_price'] ? $row['goods_price'] : $promote_price;
            $arr[$temp_index]['promote_price'] = $promote_price;
        } else {
            $promote_price = $row['goods_price'] < $row['shop_price'] ? $row['goods_price'] : 0;
            $arr[$temp_index]['promote_price'] = $promote_price;
        }
