...
function assign_ur_here($cat = 0, $str = '', $module='')
{
    ...
    $ur_here    = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                   <a itemprop="item" href="./"><span itemprop="name">' . $GLOBALS['_LANG']['home'] . '</span></a>
        ...
        if (in_array($module, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                {
                    $page_title = htmlspecialchars($val['cat_name']) . ' - ' . $shop_name;
                    $args       = array($key => $val['cat_id']);
                    $position = $start +($total - $k);
                    /** gia tri cuoi cung se chuyen thanh h1 doi voi category */
                    $allow_h1 = array('category', 'article_cat');
                    if(in_array($module,$allow_h1) && $total+1 == $position){
                        $rname = '<span itemprop="name">' . htmlspecialchars($val['cat_name']) . '</span>';
                    }else{
                        $rname = '<span itemprop="name">' . htmlspecialchars($val['cat_name']) . '</span>';
                    }
                    $url_link = build_uri($type, $args, $val['cat_name']);
                    $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">';
                    if(!empty($url_link)){
                        $ur_here   .='<a itemprop="item" href="'. $url_link . '">'.$rname.'</a>';
                    }else{
                        $ur_here   .='<a itemprop="item" href="./">'.$rname.'</a>';
                    }
                    $ur_here    .=   '<meta itemprop="position" content="'. $position.'"></li>';
        ...
        elseif ('topic' == $module)
        {
            ...
            $page_title  = $topic_name . ' - ' . $shop_name;
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
            <a itemprop="item" href="khuyen-mai"><span itemprop="name">Khuyến mãi</span></a>
        ...
        if (!empty($str))
        {
            $page_title  = $str . ' - ' . $shop_name;
            $position = $position+1;
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                    <a itemprop="item" ><span itemprop="name">' . $str. '</span></a><meta itemprop="position" content="'. $position.'"></li>';
        }
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 3 AND a.is_open = 1 ' .
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='', $recommend = '')
{
    ...
    {
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type, 'rc' => $recommend);
            break;
        case 'article_cat':
...
function assign_comment($id, $type, $page = 1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['comment_id']]['reply'] = getComments_reply($row['comment_id']);
...
function getComments_reply($parent_id){
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
        ...
        {
            $arr[$row['comment_id']]['re_content']  = nl2br(str_replace('\n', '<br />', htmlspecialchars($row['content'])));
            $arr[$row['comment_id']]['re_add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
            $arr[$row['comment_id']]['re_email']    = $row['email'];
            $arr[$row['comment_id']]['re_username'] = $row['user_name'];
            $arr[$row['comment_id']]['isadmin'] = $row['is_admin'];
        }
    return $arr;
}
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('link_fanpage',  $GLOBALS['_CFG']['qq']);
    $smarty->assign('des_home',  $GLOBALS['_CFG']['des_home']);
    $smarty->assign('shop_notice',  $GLOBALS['_CFG']['shop_notice']);
    $smarty->assign('link_youtube',  $GLOBALS['_CFG']['ww']);
    $smarty->assign('link_zalo',     $GLOBALS['_CFG']['ym']);
    $smarty->assign('link_message',  $GLOBALS['_CFG']['msn']);
...
function get_library_number($library, $template = null)
{
    ...
    if (empty($template))
    {
        $template = $GLOBALS['_module']; /* edit by ecshopvietnam */
    }
