...
/* Ghi logs phục vụ test code */

function addlog($filename, $str = ''){
    $filename = ROOT_PATH.'temp/logs/'.$filename;
    if (!file_exists($filename)) @mkdir(ROOT_PATH.'temp/logs/', 0755);
    @file_put_contents($filename, date('Y-m-d H:i:s').' - ' .$str."\n", FILE_APPEND);
}


/**
 * Thay đổi đuôi mở rộng của file
 * @param  [type] $source [description]
 * @param  string $ext    [description]
 * @return [type]         [description]
 */
function convertExtension($source, $ext = ''){
    if(!empty($ext)){
...
/**
 * Check số điện thoại diđộng Việt Nam
 * Bắt đầu là 0, có độ dài 10,11 kí tự
 * @param  string  $tel
 * @return boolean
 */
function is_mobile($mobile_number){
    return preg_match('/^0\d{9}$/i', $mobile_number);
}
...
function init_users($arr = [])
{
    ...
    $cfg = !empty($GLOBALS['_CFG']['integrate_config'])? unserialize($GLOBALS['_CFG']['integrate_config']) : [];
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_home, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
...
function clear_tpl_files($is_cache = true, $ext = '')
{
    ...
    if ($is_cache)
    {
        ...
        $cache_dir = ROOT_PATH . $tmp_dir . '/caches/'. $GLOBALS['_CFG']['template'].'/';
        $cache_dirm = ROOT_PATH . $tmp_dir . '/caches/'. $GLOBALS['_CFG']['template_mobile'].'/';
    ...
    else
    {
        ...
        $dirs[] = ROOT_PATH . $tmp_dir . '/compiled/'. $GLOBALS['_CFG']['template'].'/';
        $dirs[] = ROOT_PATH . $tmp_dir . '/compiled/'. $GLOBALS['_CFG']['template_mobile'].'/';
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                ...
                if (isset($rc) && !empty($rc))
                {
                 ...
                 /* từ khóa  khi seach chọn tìm sp ở build cấu hình */
                if (isset($keywords) && !empty($keywords))
                {
                    $pos=strpos($uri,"?");
                    $uri .= ($pos?'&amp;' : '?') .'keywords='.$keywords;
                }
            }
        ...
        case 'goods':
            ...
            {
                $slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$gid' and module='goods'");
                //$uri = $slug.'-'.$gid.'.html';
                $parent_id   = $GLOBALS['db']->getOne("SELECT parent_id FROM " .$GLOBALS['ecs']->table('category'). " WHERE cat_id = (SELECT cat_id FROM " .$GLOBALS['ecs']->table('goods'). " WHERE goods_id = '$gid')");
                if($parent_id == 0){
                    $parent_slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = (SELECT cat_id FROM " .$GLOBALS['ecs']->table('goods'). " WHERE goods_id = '$gid') AND module='category'");
                }else{
                    $parent_slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$parent_id' and module='category'");
                }
                $uri = $parent_slug.'/'.$slug;
        ...
        case 'brand':
            ...
            else
            {
                ...
                if (!empty($sort) && !empty($order) && $order != 'ASC')
                {
                    $pos=strpos($uri,"?");
                    $uri .= ($pos?'&amp;' : '?') .'sort=' . $sort;
                }
        ...
        case 'article_cat':
            ...
            else
            {
                ...
                $uri = $acid == 8 ? 'tin-tuc' : 'tin-tuc/'.$slug.'/'.$acid;
        ...
        case 'article':
            ...
            else
            {
                ...
                $parent_id = $GLOBALS['db']->getOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('article_cat'). " WHERE cat_id= $article[cat_id]");
                if($parent_id == 3){
                    $uri = 'thong-tin/'.$article["slug"].'-'.$aid.'.html';
                }else{
                    $uri = 'tin-tuc/'.$article["slug"].'-'.$aid.'.html';
                }
