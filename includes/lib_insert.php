    ...
    foreach ($res AS $key=>$row)
    {
        ...
        switch ($row['media_type'])
        {
            case 0: // Images
                ...
                $lazyload = $row['lazyload'] == 1 ? 'class="lazyload" data-': '';
                $webplazyload = $row['lazyload'] == 1 ? 'class="lazyload"': '';
                $src = $row['ad_code'];

                if(strpos($src, 'http://') === false && strpos($src, 'https://') === false){
                    $src_full  = CDN_PATH . "/afficheimg/$src";
                    $path_src = ROOT_PATH.$src_full;
                    $webp_path = convertExtension($path_src, 'webp');
                    if(file_exists($webp_path)){
                        $webp_path = str_replace(ROOT_PATH, '', $webp_path);
                        $img_display = '<picture>';
                        $img_display .= "  <source $webplazyload srcset='$webp_path' height='$row[ad_height]' alt='$row[ad_name]' type='image/webp'>";
                        $img_display .= "<img  ".$lazyload."src='$src_full' width='" .$row['ad_width']. "' height='$row[ad_height]' alt='$row[ad_name]' />";
                        $img_display .= '</picture>';
                    }else{
                        $img_display = "<img  ".$lazyload."src='$src_full' width='" .$row['ad_width']. "' height='$row[ad_height]' alt='$row[ad_name]' />";
                    }
                ...
                if(!empty($row["ad_link"])){
                    $ads[$key]['code']  = "<a href='".$row["ad_link"]."'>".$img_display."</a>";
                    $ads[$key]['link']  = $row["ad_link"];
                }else{
                    $ads[$key]['code']  = $img_display;
                    $ads[$key]['link']  = '';
                }
