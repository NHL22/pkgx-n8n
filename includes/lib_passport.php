...
function check_userinfo($user_name, $email)
{
    ...
    {
        ecs_header("Location: thanh-vien?act=get_password\n");
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['ecs']->url() . 'thanh-vien?act=get_password&uid=' . $uid . '&code=' . $code;
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $GLOBALS['ecs']->url() . 'thanh-vien?act=validate_email&hash=' . $hash;
