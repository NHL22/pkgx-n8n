...
function real_ip()
{
    ...
    setcookie("real_ipd", $realip, time()+36000, $GLOBALS['cookie_path'],$GLOBALS['cookie_domain'],$GLOBALS['cookie_secure'],true);
    return $realip;
}
...
function log_write($arg, $file = '', $line = '')
{
    ...
    file_put_contents(ROOT_PATH  . 'temp/logs/log.txt', $str, FILE_APPEND);
}
...
function ecs_geoip($ip)
{
    ...
    if ($fp === NULL)
    {
        $fp = fopen(ROOT_PATH .CDN_PATH.'/data/ipdata.dat', 'rb');
        if ($fp === false)
        {
...
function read_static_cache($cache_name)
{
    ...
    if (file_exists($cache_file_path))
    {
        ...
        /**
         * Check cache có thời hạn nobj
         */
        if(isset($time) && time() < $time){
            $result[$cache_name] = $data;
         }else{
            @unlink($cache_file_path);
            $result[$cache_name] = false;
         }
 ...
 * @return
 */
function write_static_cache($cache_name, $caches, $cache_lifetime = 3600)
{
    if ((DEBUG_MODE & 2) == 2)
    {
        return false;
    ...
    /** Add Nobj - Cache có thời hạn */
    if(isset($cache_lifetime) && $cache_lifetime > 0){
    $content .= "\$time = " . (time() + $cache_lifetime) . ";\r\n";
    }
    $content .= "?>";
 ...
 * @return
 */
function write_static_data($file_name, $data)
{
    $cache_file_path = ROOT_PATH . DATA_DIR. '/' . $file_name . '.php';
    $content = "<?php\r\n";
    $content .= '/* Write Time on '.date('Y-m-d H:i:s').' */'. "\r\n";
 ...
 * @return  array   $data
 */
function read_static_data($cache_name)
{
    static $result = array();
    if (!empty($result[$cache_name]))
    {
    ...
    $cache_file_path = ROOT_PATH . DATA_DIR. '/' . $cache_name . '.php';
    if (file_exists($cache_file_path))
    {
        include_once($cache_file_path);
        $result[$cache_name] = $data;
 ...
 */
function del_static_data($cache_name)
{
    $cache_file_path = ROOT_PATH . DATA_DIR. '/' . $cache_name . '.php';
    if (file_exists($cache_file_path))
    {
