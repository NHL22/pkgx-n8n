...
function get_collection_goods($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, '.
                ...
                'g.promote_price, g.is_hot, g.promote_start_date,g.promote_end_date, c.rec_id, c.is_attention' .
...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
            ...
            {
                $rows['handle'] = '<a href="thanh-vien?act=pay&id='.$rows['id'].'&pid='.$pid.'">'.$GLOBALS['_LANG']['pay'].'</a>';
            }
