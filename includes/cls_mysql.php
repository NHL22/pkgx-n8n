...
class cls_mysql
{
        ...
        if ($dbname)
        {
            if (mysqli_select_db($this->link_id, $dbname) === false )
            {
                if (!$quiet)
                {
                    ...
                    //throw new RuntimeException("Can't select MySQL database($dbname)!", 1);
                }
    ...
    function query($sql, $type = '')
    {
        ...
        if (!($query = mysqli_query($this->link_id, $sql)) && $type != 'SILENT')
        {
            ...
            $this->ErrorMsg();
            //throw new Exception("MySQL Query Error : $this->error_message[]['error'] - ".$sql, 1);
            return false;
        }

        // if (defined('DEBUG_MODE') && (DEBUG_MODE & 8) == 8)
        // {
        //     $log_query_path = $this->root_path  . 'temp/logs';
        //     if (!file_exists($log_query_path)){
        //         @mkdir($log_query_path, 0755, true);
        //         @chmod($_cache_dir, 0755);
        //     }
        //     $logfilename = $log_query_path  . '/mysqli_query_' . $this->dbhash . '_' . date('Y_m_d') . '.log';
        //     $str = $sql . "\n\n";
