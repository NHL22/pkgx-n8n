...
function checkoutInit(){
    ...
    /* Tự check cho nếu ko có cookie */
    var shippingMethod = $('input[id^="shipping_method_"]:checked');
    var paymentMethod = $('input[id^="payment_method_"]:checked');
    if(shippingMethod.length == 0) {
        $('input[id^="shipping_method_"]').eq(0).attr('checked','checked');;
    }
    if(paymentMethod.length == 0) {
        $('input[id^="payment_method_"]').eq(0).attr('checked','checked');;
    }
	//input.after('<em class="status"></em>');
	...
	// 	if ($(this).parent().find('.intro').length > 0) {
