...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ads',
        array('ad_link' => $ad_link, 'ad_name' => $ad_name, 'start_time' => $start_time,
            'end_time' => $end_time, 'enabled' => 1, 'ad_sort' => 1));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $ad_sort = !empty($_POST['ad_sort'])    ? intval($_POST['ad_sort'])  : 1;
    ...
    if ($_POST['media_type'] == '0')
    {
        if ((isset($_FILES['ad_img']['error']) && $_FILES['ad_img']['error'] == 0) || (!isset($_FILES['ad_img']['error']) && isset($_FILES['ad_img']['tmp_name'] ) &&$_FILES['ad_img']['tmp_name'] != 'none'))
        {
            ...
            /* Convert to Webp */
            $image->convertWebp(ROOT_PATH. DATA_DIR . '/afficheimg/'.$ad_code);
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$ecs->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,ad_sort,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($type == 0)
    {
        if ((isset($_FILES['ad_img']['error']) && $_FILES['ad_img']['error'] == 0) || (!isset($_FILES['ad_img']['error']) && isset($_FILES['ad_img']['tmp_name']) && $_FILES['ad_img']['tmp_name'] != 'none'))
        {
            $img_new_name = build_slug($_POST['ad_name']).'-'. time();
            $img_up_info = basename($image->upload_image($_FILES['ad_img'], 'afficheimg', $img_new_name));
            $ad_code = "ad_code = '".$img_up_info."'".',';
            /* Convert to Webp */
            $image->convertWebp(ROOT_PATH. DATA_DIR . '/afficheimg/'.$img_up_info);

            /* Xóa phiên bản webp trước bản bình thường */
            $webp_ads = convertExtension(ROOT_PATH. DATA_DIR . '/afficheimg/'.$_POST['old_ad_img'], 'webp');
            @unlink($webp_ads);
            /* Del old img ads */
        ...
        {
            //$ad_code = '';
            $ad_code = "ad_code = '".$_POST['old_ad_img']."'".',';
        }
        if (!empty($_POST['img_url']))
        {
   ...
   /* 提示信息 */
   $href[] = array('text' => $_LANG['back_ads_list'], 'href' => 'ads.php?act=list&pid='.intval($_POST['position_id']));
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $id = intval($_GET['id']);
    $ad_code = $exc->get_fields($id, "ad_code, position_id");
    ...
    {
        $img_name = ROOT_PATH. DATA_DIR . '/afficheimg/'.basename($img);
        /* Xóa phiên bản webp trước bản bình thường */
        $webp_ads = convertExtension($img_name, 'webp');
        @unlink($webp_ads);
        @unlink($img_name);
    }
    ...
    $url = 'ads.php?act=query&pid='.$ad_code['position_id'].'&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
