...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $goods = array(
            'goods_id'      => 0,
            'goods_desc'    => '',
            'gift_start_date'    =>  local_date('Y-m-d H:i'),
            'gift_end_date'    => local_date('Y-m-d H:i', local_strtotime('+1 month')),
            'goods_shopee'    => '',
            'goods_gifts'    => '',
    ...
    else
    {
        ...
        if (empty($goods) === true)
        {
            ...
            $goods = array(
                ...
                'goods_desc'    => '',
                'gift_start_date'    => local_date('Y-m-d H:i'),
                'gift_end_date'    => local_date('Y-m-d H:i', gmstr2tome('+1 month')),
                'goods_shopee'    => '',
                'goods_gifts'    => '',
                'cat_id'        => 0,
                'is_on_sale'    => '1',
        ...
        if(!empty($goods['goods_type'])){
            ...
            $goods['specifications_id'] = $_attribute = '';
        }
        ...
        if (isset($goods['time_gift']) && $goods['time_gift'] == '0')
        {
            unset($goods['gift_start_date']);
            unset($goods['gift_start_date']);
        }
        else{
            $goods['gift_start_date'] = local_date('Y-m-d H:i', $goods['gift_start_date']);
            $goods['gift_end_date'] = local_date('Y-m-d H:i', $goods['gift_end_date']);
        }
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price, display_type, display_value " .
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]' ORDER BY type, sort ASC";
        ...
        if($goods['goods_type'] > 0){
            $sql= "SELECT ga.goods_attr_id, ga.attr_id, ga.attr_value FROM ". $ecs->table('attribute') . " AS a ".
                  " LEFT JOIN ". $ecs->table('goods_attr') . "  AS ga ON a.attr_id=ga.attr_id ".
                  " WHERE a.attr_color=1 and ga.goods_id='" . $goods['goods_id']. "' ORDER BY ga.goods_attr_id ";
            $res = $db->getAll($sql);
            $color_list = array();
            foreach ($res as $key => $row) {
                $color_list[$row['goods_attr_id']] = $row['attr_value'];
            }
    ...
    /* 创建 html editor */
    CKeditor('goods_desc', $goods['goods_desc']);
    CKeditor('goods_brief', $goods['goods_brief']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $name_seo = build_slug($goods_name);
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        if ($_REQUEST['goods_id'] > 0)
        {
            ...
            $sql = "SELECT goods_thumb, goods_img, original_img " .
                    ...
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
            $row = $db->getRow($sql);
            if ($row['goods_thumb'] != '' && is_file(ROOT_PATH.CDN_PATH.'/'. $row['goods_thumb']))
            {
                $webp_goods_thumb  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['goods_thumb'], 'webp');
                @unlink($webp_goods_thumb);
                @unlink(ROOT_PATH.CDN_PATH.'/'. $row['goods_thumb']);
            }
            if ($row['goods_img'] != '' && is_file(ROOT_PATH.CDN_PATH.'/'. $row['goods_img']))
            {
                $webp_goods_img  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['goods_img'], 'webp');
                @unlink($webp_goods_img);
                @unlink(ROOT_PATH.CDN_PATH.'/'. $row['goods_img']);
            }
            if ($row['original_img'] != '' && is_file(ROOT_PATH.CDN_PATH.'/'. $row['original_img']))
            {
                /* 先不处理，以防止程序中途出错停止 */
        ...
        if ($_CFG['auto_generate_gallery'])
        {
            ...
            $newname    = dirname($img) . '/' . $name_seo.'-'.time() . $itype;
            if (!copy(ROOT_PATH.CDN_PATH.'/'. $img, ROOT_PATH.CDN_PATH.'/'. $newname))
            {
        ...
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) || $is_url_goods_img)
        {
            ...
            if (empty($is_url_goods_img))
            {
                ...
                {
                    $goods_img = $image->make_thumb(ROOT_PATH.CDN_PATH.'/'. $goods_img , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                    if ($goods_img === false)
                    {
                        sys_msg($image->error_msg(), 1, array(), false);
                    }
                ...
                if ($_CFG['auto_generate_gallery'])
                {
                    ...
                    $newname = dirname($img) . '/' . $name_seo.'2-'.time() . $itype;
                    if (!copy(ROOT_PATH.CDN_PATH.'/'. $img, ROOT_PATH.CDN_PATH.'/'. $newname))
                    {
                        sys_msg('fail to copy file: ' . realpath('../' . $img), 1, array(), false);
                    }
            ...
            if ($_CFG['auto_generate_gallery'])
            {
                ...
                {
                    $gallery_thumb = $image->make_thumb(ROOT_PATH.CDN_PATH.'/' . $img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height']);
    ...
    //var_dump($original_img, $goods_img,$gallery_img,$gallery_thumb);exit;
    ...
    else
    {
        ...
        if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img))
        {
            ...
            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
            {
                $goods_thumb = $image->make_thumb(ROOT_PATH.CDN_PATH.'/' . $original_img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height']);
                if ($goods_thumb === false)
                {
                    sys_msg($image->error_msg(), 1, array(), false);
                }
    ...
    if (!empty($is_url_goods_img))
    {
        unlink(ROOT_PATH . CDN_PATH.'/'. $original_img);
        empty($newname) || unlink(ROOT_PATH .CDN_PATH.'/'. $newname);
    ...
    $deal_price = !empty($_POST['deal_price']) ? $_POST['deal_price'] : 0;
    $partner_price = !empty($_POST['partner_price']) ? $_POST['partner_price'] : 0;
    ...
    /* add by Nobj */
    $goods_name_custom    = isset($_POST['goods_name_custom']) ? filter_var($_POST['goods_name_custom'], FILTER_SANITIZE_STRING) : '';
    ...
    $seller_note = isset($_POST['seller_note']) ? filter_var($_POST['seller_note'], FILTER_SANITIZE_STRING) : '';
    $goods_desc  = isset($_POST['goods_desc']) ? trim($_POST['goods_desc']) : '';
    $goods_shopee = isset($_POST['goods_shopee']) ? filter_var($_POST['goods_shopee'], FILTER_SANITIZE_STRING) : '';

    $goods_gifts = isset($_POST['goods_gifts']) ? filter_var($_POST['goods_gifts'], FILTER_SANITIZE_STRING) : '';
    $time_gift = !empty($_POST['time_gift']) ? floatval($_POST['time_gift'] ) : 0;
    $is_gift = empty($time_gift) ? 0 : 1;
    $gift_start_date = ($is_gift && !empty($_POST['gift_start_date'])) ? local_strtotime($_POST['gift_start_date']) : 0;
    $gift_end_date = ($is_gift && !empty($_POST['gift_end_date'])) ? local_strtotime($_POST['gift_end_date']) : 0;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, meta_robots, template_file, time_gift,goods_shopee, goods_gifts, gift_start_date, gift_end_date, goods_name_custom, meta_title, meta_desc, goods_name_style, goods_sn, " .
            "cat_id, brand_id, shop_price, deal_price, partner_price, market_price, virtual_sales, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, goods_thumb_url, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$goods_name', '$meta_robots', '$template_file', '$time_gift', '$goods_shopee','$goods_gifts', '$gift_start_date', '$gift_end_date', '$goods_name_custom', '$meta_title', '$meta_desc', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                "'$brand_id', '$shop_price', '$deal_price', '$partner_price','$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$goods_thumb_url', '$original_img', ".
                    "'$keywords', '$goods_brief', '$seller_note', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$goods_desc', '" . time() . "', '". time() ."', '$goods_type', '$rank_integral', '$suppliers_id')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, meta_robots, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price,deal_price, partner_price,  market_price, virtual_sales, is_promote, promote_price, " .
                ...
                "VALUES ('$goods_name', '$meta_robots', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price',  '$deal_price', '$partner_price','$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$keywords', '$goods_brief', '$seller_note', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$goods_desc', '" . time() . "', '". time() ."', '$goods_type', '$code', '$rank_integral')";
        }
    }
    else
    {
        /* Nếu có hình ảnh cập nhật được tải lên, hãy xóa hình ảnh sản phẩm gốc */
        $sql = "SELECT goods_thumb, goods_img, original_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
        if ($proc_thumb && $goods_img && $row['goods_img'] && !goods_parse_url($row['goods_img']))
        {
            ...
            $webp_original_img  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['goods_img'], 'webp');
            @unlink($webp_original_img);

            @unlink(ROOT_PATH .CDN_PATH.'/'. $row['goods_img']);
            @unlink(ROOT_PATH .CDN_PATH.'/'. $row['original_img']);
        }
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "goods_shopee = '$goods_shopee', " .
                "goods_gifts = '$goods_gifts', " .
                "goods_name_custom = '$goods_name_custom', " .
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
                ...
                $display_type = $_POST['display_type_list'][$key];
                $display_value = $_POST['display_value_list'][$key];
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['display_type'] = $display_type;
                        $goods_attr_list[$attr_id][$attr_value]['display_value'] = $display_value;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price, display_type, display_value)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]', '$info[display_type]', '$info[display_value]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]', display_type = '$info[display_type]', display_value = '$info[display_value]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
    ...
    $original_img = reformat_image_name('goods', $goods_id, $original_img, 'source', $name_seo);
    $goods_img = reformat_image_name('goods', $goods_id, $goods_img, 'goods', $name_seo);
    $goods_thumb = reformat_image_name('goods_thumb', $goods_id, $goods_thumb, 'thumb', $name_seo);
    // var_dump($original_img,$goods_img);
    // exit;
    if ($goods_img !== false)
    {
   ...
   /* thay đổi tên gallery */
    if (isset($img))
    {
        ...
        if (empty($is_url_goods_img))
        {
            $img = reformat_image_name('gallery', $goods_id, $img, 'source', $name_seo);
            $gallery_img = reformat_image_name('gallery', $goods_id, $gallery_img, 'goods', $name_seo);
        }
        else
        {
            $img = $url_goods_img;
        ...
        $gallery_thumb = reformat_image_name('gallery_thumb', $goods_id, $gallery_thumb, 'thumb', $name_seo);
    ...
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'], $name_seo, $_POST['sort'], $_POST['type']);
    ...
    if (!$is_insert)
    {
        ...
        if(isset($_POST['old_sort'])){
            foreach ($_POST['old_sort'] AS $img_id => $val)
            {
                $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET sort = '$val' WHERE img_id = '$img_id' LIMIT 1";
                $db->query($sql);
            }
        }
        if(isset($_POST['old_type'])){
            foreach ($_POST['old_type'] AS $img_id => $val)
            {
                $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET type = '$val' WHERE img_id = '$img_id' LIMIT 1";
                $db->query($sql);
            }
        }
        if(isset($_POST['old_goods_attr_id'])){
            foreach ($_POST['old_goods_attr_id'] AS $img_id => $val)
            {
                if($val > 0){
                    $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET goods_attr_id = $val WHERE img_id = $img_id AND type = 1 LIMIT 1";
                    $db->query($sql);
                }
            }
        }
    ...
    if ($proc_thumb && !$_CFG['retain_original_img'] && !empty($original_img))
    {
        ...
        @unlink(ROOT_PATH.CDN_PATH.'/'. $original_img);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $img);
    }

    /* 记录上一次选择的分类和品牌 */
    setcookie('ECSCP[last_choose]', $catgory_id . '|' . $brand_id, time() + 86400, NULL, NULL, NULL, TRUE);
    /* 清空缓存 */
    clear_cache_files();

    /* Cấu hình tồn kho theo thuộc tính */
...
elseif ($_REQUEST['act'] == 'edit_seller_note')
{
    check_authz_json('goods_manage');

    $goods_id   = intval($_POST['id']);
    $seller_note = json_str_iconv(trim($_POST['val']));

    if ($exc->edit("seller_note = '$seller_note', last_update=" .time(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($seller_note));
    }
}
...
elseif ($_REQUEST['act'] == 'edit_deal_price')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $deal_price    = floatval($_POST['val']);

    if ($deal_price < 0 || $deal_price == 0 && $_POST['val'] != "$deal_price")
    {
        make_json_error('Giá không hợp lệ');
    }
    else
    {
        if ($exc->edit("deal_price = '$deal_price', last_update=" .time(), $goods_id))
        {
            clear_cache_files();
            make_json_result(number_format($deal_price, 2, '.', ''));
        }
    }
}

elseif ($_REQUEST['act'] == 'edit_partner_price')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $partner_price    = floatval($_POST['val']);

    if ($partner_price < 0 || $partner_price == 0 && $_POST['val'] != "$partner_price")
    {
        make_json_error('Giá khôn hợp lệ');
    }
    else
    {
        if ($exc->edit("partner_price = '$partner_price', last_update=" .time(), $goods_id))
        {
            clear_cache_files();
            make_json_result(number_format($partner_price, 2, '.', ''));
        }
    }
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'toggle_is_home')
{
    check_authz_json('goods_manage');
    $goods_id       = intval($_POST['id']);
    $is_home         = intval($_POST['val']);
    if ($exc->edit("is_home = '$is_home', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($is_home);
    }
}
/*------------------------------------------------------ */
//-- 修改上架状态
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    {
        $webp_goods_thumb  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$goods['goods_thumb'], 'webp');
        @unlink($webp_goods_thumb);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $goods['goods_thumb']);
    }
    if (!empty($goods['goods_img']))
    {
        $webp_goods_img  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$goods['goods_img'], 'webp');
        @unlink($webp_goods_img);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $goods['goods_img']);
    }
    if (!empty($goods['original_img']))
    {
        $webp_original_img  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$goods['goods_img'], 'webp');
        @unlink($webp_original_img);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $goods['original_img']);
    }
    /* 删除商品 */
    $exc->drop($goods_id);
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        {
            $webp_img_url  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['img_url'], 'webp');
            @unlink($webp_img_url);
            @unlink(ROOT_PATH.CDN_PATH.'/'. $row['img_url']);
        }
        if (!empty($row['thumb_url']))
        {
            $webp_thumb_url  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['thumb_url'], 'webp');
            @unlink($webp_thumb_url);
            @unlink(ROOT_PATH.CDN_PATH.'/'. $row['thumb_url']);
        }
        if (!empty($row['img_original']))
        {
            $webp_img_original  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['img_original'], 'webp');
            @unlink($webp_img_original);
            @unlink(ROOT_PATH.CDN_PATH.'/'. $row['img_original']);
        }
    }
    ...
    $db->query("DELETE FROM " . $ecs->table('slug') . " WHERE id = '$goods_id' AND module='goods' LIMIT 1");


    $db->query("DELETE FROM " . $GLOBALS['ecs']->table('slug') . " WHERE id = $goods_id AND module='goods' LIMIT 1");
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    if ($row['img_url'] != '' && is_file(ROOT_PATH.CDN_PATH.'/'. $row['img_url']))
    {
        $webp_img_url  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['img_url'], 'webp');
        @unlink($webp_img_url);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $row['img_url']);
    }
    if ($row['thumb_url'] != '' && is_file(ROOT_PATH.CDN_PATH.'/'. $row['thumb_url']))
    {
        $webp_thumb_url  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['thumb_url'], 'webp');
            @unlink($webp_thumb_url);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $row['thumb_url']);
    }
    if ($row['img_original'] != '' && is_file(ROOT_PATH.CDN_PATH.'/'. $row['img_original']))
    {
        $webp_img_original  = convertExtension(ROOT_PATH.CDN_PATH. '/'.$row['img_original'], 'webp');
        @unlink($webp_img_original);
        @unlink(ROOT_PATH.CDN_PATH.'/'. $row['img_original']);
