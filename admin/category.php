...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $cat['is_tag']  = !empty($_POST['is_tag'])? intval($_POST['is_tag']) : 0;
    $cat['show_in_home']  = !empty($_POST['show_in_home'])? intval($_POST['show_in_home']) : 0;
    $cat['meta_robots']  = !empty($_POST['meta_robots'])? $_POST['meta_robots'] : 'INDEX,FOLLOW';
    $cat['meta_title']  = !empty($_POST['meta_title'])? trim($_POST['meta_title']) : '';
    $cat['meta_desc']  = !empty($_POST['meta_desc'])? trim($_POST['meta_desc']) : '';
    $cat['long_desc']  = !empty($_POST['long_desc'])? trim($_POST['long_desc']) : '';
    $cat['ads_category']  = !empty($_POST['ads_category'])? intval($_POST['ads_category']) : 0;
    $cat['ads_category_mobile']  = !empty($_POST['ads_category_mobile'])? intval($_POST['ads_category_mobile']) : 0;
    $cat['ads_home']  = !empty($_POST['ads_home'])? intval($_POST['ads_home']) : 0;
    $cat['ads_home_mobile']  = !empty($_POST['ads_home_mobile'])? intval($_POST['ads_home_mobile']) : 0;
    /* End Ext */
    ...
    if ($db->autoExecute($ecs->table('category'), $cat) !== false)
    {
        ...
        create_slug($cat_id, 'category', true);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    CKeditor('cat_desc',  $cat_info['cat_desc']);
...
if ($_REQUEST['act'] == 'update')
{
    ...
    {
         update_slug($cat_id, 'category',true);
        //更新首页推荐
        insert_cat_recommend($cat['cat_recommend'], $cat_id);
        /* 更新分类信息成功 */
        clear_cache_files(); // 清除缓存
...
if ($_REQUEST['act'] == 'toggle_is_show_home')
{
    check_authz_json('cat_manage');
    ...
    if (cat_update($id, array('show_in_home' => $val)) != false)
    {
        clear_cache_files();
        make_json_result($val);
    }
