...
// --- DỌN SẠCH BỘ ĐỆM VÀ BẮT ĐẦU API ---
@ob_end_clean();
...
// --- Lấy dữ liệu và tham số random thời gian ---
...
// Tham số mới: số ngày tối đa để lùi lại
$days_ago = isset($data['days_ago']) ? intval($data['days_ago']) : 0;
...
// --- Xử lý thời gian ---
$add_time = gmtime(); // Mặc định là thời gian hiện tại

if ($days_ago > 0) {
    // Nếu có tham số days_ago, tạo một mốc thời gian ngẫu nhiên
    $current_timestamp = time(); // Thời gian hiện tại (local)
    // Tính toán mốc thời gian xa nhất trong quá khứ
    $past_timestamp = $current_timestamp - ($days_ago * 24 * 60 * 60);
    
    // Chọn một mốc thời gian ngẫu nhiên giữa "quá khứ" và "hiện tại"
    $random_local_timestamp = mt_rand($past_timestamp, $current_timestamp);
    
    // Chuyển đổi mốc thời gian ngẫu nhiên đó sang GMT để lưu vào database
    $add_time = local_to_utc($random_local_timestamp);
}
