 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $filename ='',$sort=1,$type=1)
{
    /* 是否处理缩略图 */
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if ($proc_thumb && gd_version() > 0)
            {
                ...
                copy(ROOT_PATH.CDN_PATH.'/'. $img_original, ROOT_PATH.CDN_PATH.'/'. $newname);
                $img_url    = $newname;
            ...
            /* Chỉ tạo thumb cho gallery khi type = 1 */
            $thumb_url = $type[$key] == 1 ? reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb', $filename) : '';
            $sorts = $sort[$key]; $types = $type[$key];
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, sort, type) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original', '$sorts', '$types')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original))
            {
                ...
                @unlink(ROOT_PATH.CDN_PATH.'/'. $img_original);
            }
        }
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.display_type, v.display_value, v.attr_value, v.attr_price ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    {
        $html .= "<tr><td class='label' style='width:20%'>";
       ...
       if($val['attr_type'] == 1 || $val['attr_type'] == 2){
            ...
            $html .= $GLOBALS['_LANG']['display_type'].' <select name="display_type_list[]">
                        <option value="">Radio</option>';
            foreach ($options as $key => $row) {
                $selected = $key == $val['display_type'] ? 'selected': '';
            $html .= $val['display_type'].'<option value="'.$key.'" '.$selected.'>'.$row.'</option>';
            }
            $html .=   '</select>';
       }else{
            $html .= '<input type="hidden" name="display_type_list[]" value="" />';
       }

        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            $GLOBALS['_LANG']['display_value'].' <input type="text" name="display_value_list[]" value="' . $val['display_value'] . '" size="20" />' :
            '<input type="hidden" name="display_value_list[]" value="" />';
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, is_home, virtual_sales, deal_price, partner_price,seller_note, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
...
function reformat_image_name($type, $goods_id, $source_img, $position='', $filename = '')
{
    ...
    if ($position == 'source')
    {
        if (move_image_file(ROOT_PATH.CDN_PATH.'/'.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext))
        {
            $source_img =  'images/'.$sub_dir.'/source_img/'.$img_name.$img_ext;
            $image->convertWebp(ROOT_PATH.CDN_PATH.'/'.$source_img);
            return $source_img;
        }
    }
    elseif ($position == 'thumb')
    {
        if (move_image_file(ROOT_PATH.CDN_PATH.'/'.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext))
        {
            $thumb = 'images/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext;
            $image->convertWebp(ROOT_PATH.CDN_PATH.'/'.$thumb);
            return $thumb;
        }
    }
    else
    {
        if (move_image_file(ROOT_PATH.CDN_PATH.'/'.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext))
        {
            $goods_img = 'images/'.$sub_dir.'/goods_img/'.$img_name.$img_ext;
            $image->convertWebp(ROOT_PATH.CDN_PATH.'/'.$goods_img);
            return $goods_img;
