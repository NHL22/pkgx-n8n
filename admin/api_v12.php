...
function create_article($data)
{
    ...
    $author = isset($data['author']) ? filter_var($data['author'], FILTER_SANITIZE_STRING) : '';
    $author_email = isset($data['author_email']) ? filter_var($data['author_email'], FILTER_SANITIZE_EMAIL) : '';
    $keywords = isset($data['keywords']) ? filter_var($data['keywords'], FILTER_SANITIZE_STRING) : '';
    $description = isset($data['description']) ? filter_var($data['description'], FILTER_SANITIZE_STRING) : ''; 
    $link_url = isset($data['link_url']) ? filter_var($data['link_url'], FILTER_SANITIZE_URL) : '';
    $is_open = isset($data['is_open']) ? intval($data['is_open']) : 0;
    $add_time = time();
    $custom_title = isset($data['custom_title']) ? filter_var($data['custom_title'], FILTER_SANITIZE_STRING) : '';
    $meta_title = isset($data['meta_title']) ? filter_var($data['meta_title'], FILTER_SANITIZE_STRING) : '';
    $meta_desc = isset($data['meta_desc']) ? filter_var($data['meta_desc'], FILTER_SANITIZE_STRING) : '';
    $meta_robots = isset($data['meta_robots']) ? $data['meta_robots'] : 'INDEX,FOLLOW';
    $content = isset($data['content']) ? $data['content'] : '';
    $template_file = isset($data['template_file']) ? filter_var($data['template_file'], FILTER_SANITIZE_STRING) : '';
    $open_type = 0; // Luôn mặc định là 0
    // Lấy URL của ảnh đã được tải lên từ dữ liệu JSON
    $article_thumb = isset($data['article_thumb_url']) ? $data['article_thumb_url'] : '';
    $article_sthumb = isset($data['article_sthumb_url']) ? $data['article_sthumb_url'] : '';
    $article_mthumb = isset($data['article_mthumb_url']) ? $data['article_mthumb_url'] : '';
    ...
    $escaped_article_sthumb = mysqli_real_escape_string($db->link_id, $article_sthumb);
    $escaped_article_mthumb = mysqli_real_escape_string($db->link_id, $article_mthumb);
    ...
    // --- Tạo câu lệnh SQL INSERT và thực thi (Đã bỏ cột importance) ---
    $sql = "INSERT INTO " . $ecs->table('article') . "(title, custom_title, template_file, meta_robots, meta_title, meta_desc, article_thumb, article_sthumb, article_mthumb, cat_id, article_type, is_open, author, ".
           ...
           "VALUES ('$escaped_final_title', '$escaped_custom_title', '$escaped_template_file', '$escaped_meta_robots', '$escaped_meta_title', '$escaped_meta_desc', '$escaped_article_thumb', '$escaped_article_sthumb', '$escaped_article_mthumb', '$article_cat', '$article_type', '$is_open','$escaped_author', '$escaped_author_email', '$escaped_keywords', '$escaped_content', '$add_time', '0', '$escaped_link_url', '$escaped_description')";
    ...
    if ($db->query($sql)) {
        ...
        // Cập nhật bảng goods_article (bảng liên kết sản phẩm) nếu cần
        $update_goods_sql = "UPDATE " . $ecs->table('goods_article') . " SET article_id = '$article_id' WHERE article_id = 0";
        $db->query($update_goods_sql);
        write_log("Updated goods_article table with article_id: " . $article_id);
