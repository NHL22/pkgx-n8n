...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    CKeditor('brand_cat_desc', '');
    CKeditor('brand_long_desc', '');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $brand_desc = !empty($_POST['brand_desc']) ? strip_tags(trim($_POST['brand_desc'])) : '';

    $brand_name = strip_tags(trim($_POST['brand_name']));
    $friendly_name = build_slug($brand_name);
    $brand_logo = !empty($_POST['brand_logo']) ? trim($_POST['brand_logo']) : '';

    $site_url = !empty($_POST['slug']) ? trim($_POST['slug']) : '';

    $meta_desc = !empty($_POST['meta_desc']) ? strip_tags(trim($_POST['meta_desc'])) : '';
    $brand_long_desc = !empty($_POST['brand_long_desc'])? trim($_POST['brand_long_desc']) : '';
    $brand_cat_desc = !empty($_POST['brand_cat_desc'])? trim($_POST['brand_cat_desc']) : '';


    $sort_order = intval($_POST['sort_order']);
    $is_hot = intval($_POST['is_hot']);

    $sql = "INSERT INTO ".$ecs->table('brand')."(
                    brand_name, site_url, brand_desc,  brand_long_desc, brand_cat_desc, brand_logo, is_show, is_hot, sort_order) ".
           "VALUES (
                '$brand_name', '$site_url', '$brand_desc', '$brand_long_desc','$brand_cat_desc', '$brand_logo', '$is_show', '$is_hot', '$sort_order')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('slug', get_slug($_REQUEST['id'], 'brand'));
    CKeditor('brand_cat_desc',  $brand['brand_cat_desc']);
    CKeditor('brand_long_desc', $brand['brand_long_desc']);
...
$brand_desc = strip_tags(trim($_POST['brand_desc']));
    ...
    $param = "brand_name = '$brand_name',brand_desc = '$brand_desc',brand_cat_desc = '$brand_cat_desc',brand_long_desc = '$brand_long_desc', brand_logo = '$brand_logo', is_hot = '$is_hot', site_url='$site_url', brand_desc='$brand_desc', is_show='$is_show', sort_order='$sort_order' ";
    ...
    if ($exc->edit($param,  $_POST['id']))
    {
        update_slug($_POST['id'], 'brand');
         //@unlink(ROOT_PATH.DATA_DIR.'/brandlogo/'.$_POST['old_brandlogo']);
