...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $sql="SELECT `ec_salt` FROM ". $ecs->table('admin_user') ."WHERE ";
    if(is_email($_POST['username'])){
       $sql .= " email = '" . $_POST['username']."'";
    }else{
    ...
    if(!empty($ec_salt))
    {
         ...
         $sql = "SELECT user_id, user_name, password, add_time, action_list, last_login,suppliers_id,ec_salt,passport_uid".
            ...
            " WHERE(user_name = '" . $_POST['username']. "' OR email = '" . $_POST['username']. "') AND password = '" . md5(md5($_POST['password']).$ec_salt) . "'";
    }
    else
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, add_time, action_list, last_login,suppliers_id,ec_salt,passport_uid".
            ...
            " WHERE (user_name = '" . $_POST['username']. "' OR email = '" . $_POST['username']. "') AND password = '" . md5($_POST['password']) . "'";
    }
    $row = $db->getRow($sql);
