...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        $topic = array('title' => '', 'topic_intro' => '', 'topic_type' => 0, 'url' => '','meta_robots'=>'NOINDEX,NOFOLLOW', 'topic_img'=> '','topic_img_mobile'=> '');
        $smarty->assign('topic', $topic);
        $smarty->assign('slug', '');
        CKeditor('topic_intro', $topic['topic_intro']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $topic_img= empty($_POST['topic_img']) ? '' : $_POST['topic_img'];
    $topic_img_mobile= empty($_POST['topic_img_mobile']) ? '' : $_POST['topic_img_mobile'];
    ...
    if ($_FILES['title_pic']['name'] && $_FILES['title_pic']['size'] > 0)
    {
        ...
        $filename = $_POST['slug'].'-'.time();
        $dir = ROOT_PATH . DATA_DIR . '/afficheimg/';

        $image = new cls_image($GLOBALS['_CFG']['bgcolor']);
        $title_pic = $image->make_thumb($_FILES['title_pic']['tmp_name'], $GLOBALS['_CFG']['article_thumb_width'],$GLOBALS['_CFG']['article_thumb_height'], $dir, '', $filename);
        $title_pic = str_replace(DATA_DIR.'/', '', $title_pic);
        if ($title_pic == false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
        /* xóa hình cũ nếu có cập nhật */
        if (!$is_insert){
            @unlink(ROOT_PATH . DATA_DIR .'/'.$_POST['title_pic_old']);
        }
    }
    ...
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $ecs->table('topic') . " (title,start_time,end_time,data,intro,template,css,topic_img, topic_img_mobile,title_pic,base_style, htmls,keywords,description)" .
                "VALUES ('$topic_name','$start_time','$end_time','$data','$_POST[topic_intro]','$_POST[topic_template_file]','$_POST[topic_css]', '$topic_img', '$topic_img_mobile', '$title_pic', '$base_style', '$htmls','$keywords','$description')";
        $db->query($sql);
    ...
    {
        $ext = !empty($topic_img) ? " topic_img='$topic_img', " : '';
        $ext .= !empty($topic_img_mobile) ? " topic_img_mobile='$topic_img_mobile', " : '';
        $ext .= !empty($title_pic) ? " title_pic='$title_pic', " : '';
        $ext .= !empty($data) ? " data='$data', " : '';
        $ext .= !empty($intro_end) ? " intro_end='$intro_end', " : '';

        $sql = "UPDATE " . $ecs->table('topic') .
                "SET title='$topic_name', start_time='$start_time',end_time='$end_time',intro='$_POST[topic_intro]',template='$_POST[topic_template_file]',css='$_POST[topic_css]', base_style='$base_style', htmls='$htmls', keywords='$keywords', ".$ext." description='$description'" .
...
elseif ($_REQUEST["act"] == "delete")
{
    ...
    $thumb = "SELECT title_pic, topic_img, topic_img_mobile FROM " . $GLOBALS['ecs']->table('topic') . " WHERE ";
    ...
    $links_thumb = $db->getAll($thumb);
    foreach($links_thumb as $key => $link){
        @unlink(ROOT_PATH. DATA_DIR .'/'.$link['title_pic']);
        @unlink(ROOT_PATH. DATA_DIR .'/'.$link['topic_img']);
        @unlink(ROOT_PATH. DATA_DIR .'/'.$link['topic_img_mobile']);
    }
    /* xoa db */
