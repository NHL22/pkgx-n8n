...
if ($action == 'act_add_message')
{
    ...
    $message = array(
        ...
        'user_tel'    => isset($_POST['user_tel']) ? htmlspecialchars(trim($_POST['user_tel']))     : '',
        'msg_type'    => isset($_POST['msg_type']) ? intval($_POST['msg_type'])     : 0,
        'msg_title'   => isset($_POST['msg_title']) ? addslashes(trim($_POST['msg_title']))   : '',
        'msg_content' => isset($_POST['msg_content']) ? addslashes(trim($_POST['msg_content'])) : '',
        'order_id'    => 0,
        'msg_area'    => 1,
        'upload'      => array()
     );
    ...
    if (add_message($message))
    {
        ...
        if ($GLOBALS['_CFG']['service_email'] != '')
        {
            ...
            $msg_content .= '<p>Số ĐT: '.$telephone.' </p>';
            $msg_content .= '<p>Nội dung:</p>';
            $msg_content .= '<p>'.addslashes($message['msg_content']).'</p>';
            @send_mail($GLOBALS['_CFG']['shop_name'], $GLOBALS['_CFG']['service_email'], $user_name.' - Gửi liên hệ: '.addslashes($message['msg_title']), $msg_content, 1);
        }
