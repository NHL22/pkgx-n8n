...
if (empty($_SESSION['user_id']))
{
    if (!in_array($action, $not_login_arr))
    {
        if (in_array($action, $ui_arr))
        {
            ...
            {
                $back_act = 'thanh-vien?' . strip_tags($_SERVER['QUERY_STRING']);
            }
            $action = 'login';
        }
        else
...
if ($action == 'register')
{
    if ((!isset($back_act)||empty($back_act)) && isset($request_url))
    {
        $back_act = strpos($request_url, 'thanh-vien') ? './' : $request_url;
    }
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if(preg_match("/^1[34578]{1}\d{9}$/",$username)){
            if($sms_code != '' && $sms_code == $_SESSION[$username.'-sms_code']){
                ...
                if (time() - $_SESSION[$username.'-send_time'] > 600) {
                    show_message('Mã xác minh SMS đã hết hạn', 'Trang trước', 'thanh-vien?act=register', 'error');
                }
                $other['mobile_phone'] = $username;
                $_SESSION[$username.'-sms_code'] ='false';
            }else{
                show_message('Lỗi mã xác minh SMS', $_LANG['sign_up'], 'thanh-vien?act=register', 'error');
            }
        }
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            // }
            show_message(sprintf($_LANG['register_success'], $username . $ucdata), array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act, 'thanh-vien'), 'info');
        }
        else
        {
            $err->show($_LANG['sign_up'], 'thanh-vien?act=register');
        }
    }
...
}elseif ($action == 'validate_email')
{
    ...
    if ($hash)
    {
        ...
        if ($id > 0)
        {
            ...
            $row = $db->getRow($sql);
            show_message(sprintf($_LANG['validate_ok'], $row['user_name'], $row['email']),$_LANG['profile_lnk'], 'thanh-vien');
        }
    }
...
elseif ($action == 'act_login')
{
    ...
    if (empty($password))
    {
        show_message($_LANG['passport_js']['password_empty'], $_LANG['relogin_lnk'], 'thanh-vien', 'error');
    }
    ...
    if ($user->login($username, $password,isset($_POST['remember'])))
    {
        ...
        ecs_header("Location: thanh-vien\n");
        exit;
        //show_message($_LANG['login_success'] . $ucdata , array($_LANG['profile_lnk']), array('thanh-vien'), 'info');
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
...
elseif ($action == 'logout')
{
    ...
    $user->logout();
    $ucdata = empty($user->ucdata)? "" : $user->ucdata;
    show_message($_LANG['logout'], array($_LANG['back_home_lnk']), array('./'), 'info');
}
...
elseif ($action == 'act_edit_profile')
{
    ...
    {
        show_message($_LANG['edit_profile_success'], $_LANG['profile_lnk'], 'thanh-vien?act=profile', 'info');
    }
    else
    {
...
elseif ($action == 'check_answer')
{
    ...
    if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($_POST['captcha']))
        {
            show_message($_LANG['invalid_captcha'], $_LANG['back_retry_answer'], 'thanh-vien?act=qpassword_name', 'error');
        }
    ...
    {
        show_message($_LANG['wrong_passwd_answer'], $_LANG['back_retry_answer'], 'thanh-vien?act=qpassword_name', 'info');
    }
    else
    {
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
            ...
            $db->query($sql);
            $user->logout();
            show_message($_LANG['edit_password_success'], $_LANG['relogin_lnk'], 'thanh-vien?act=login', 'info');
        }
        else
        {
...
elseif ($action == 'act_add_bonus')
{
    ...
    {
        show_message($_LANG['add_bonus_sucess'], $_LANG['back_up_page'], 'thanh-vien?act=bonus', 'info');
    }
    else
    {
        $err->show($_LANG['back_up_page'], 'thanh-vien?act=bonus');
    }
}
...
elseif ($action == 'order_list')
{
    ...
    $pager  = get_pager('thanh-vien', array('act' => $action), $record_count, $page);
...
elseif ($action == 'cancel_order')
{
    ...
    if (cancel_order($order_id, $user_id))
    {
        ...
        ecs_header("Location: thanh-vien?act=order_list\n");
        exit;
    }
    else
    {
        $err->show($_LANG['order_list_lnk'], 'thanh-vien?act=order_list');
    }
}
...
elseif ($action == 'act_edit_address')
{
    ...
    {
        show_message($_LANG['edit_address_success'], $_LANG['address_list_lnk'], 'thanh-vien?act=address_list');
    }
}
...
elseif ($action == 'drop_consignee')
{
    ...
    {
        ecs_header("Location: thanh-vien?act=address_list\n");
        exit;
    }
    else
    {
...
elseif ($action == 'delete_collection')
{
    ...
    ecs_header("Location: thanh-vien?act=collection_list\n");
    exit;
}
...
elseif ($action == 'message_list')
{
    ...
    if ($order_id)
    {
        ...
        $order_info['url'] = 'thanh-vien?act=order_detail&order_id=' . $order_id;
    }
    else
    {
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('feedback').
    ...
    $pager = get_pager('thanh-vien', $act, $record_count, $page, 5);
...
elseif ($action == 'comment_list')
{
    ...
    $record_count = $db->getOne($sql);
    $pager = get_pager('thanh-vien', array('act' => $action), $record_count, $page, 5);
...
elseif ($action == 'act_add_message')
{
    ...
    if (add_message($message))
    {
        ...
        // }
        show_message($_LANG['add_message_success'], $_LANG['message_list_lnk'], 'thanh-vien?act=message_list&order_id=' . $message['order_id'],'info');
    }
    else
    {
        $err->show($_LANG['message_list_lnk'], 'thanh-vien?act=message_list');
    }
}
...
elseif ($action == 'act_del_tag')
{
    ...
    ecs_header("Location: thanh-vien?act=tag_list\n");
    exit;
...
elseif ($action == 'act_add_booking')
{
    ...
    {
        show_message($_LANG['booking_success'], $_LANG['back_booking_list'], 'thanh-vien?act=booking_list',
        'info');
    }
    else
    {
        $err->show($_LANG['booking_list_lnk'], 'thanh-vien?act=booking_list');
    }
}
...
elseif ($action == 'act_del_booking')
{
    ...
    if ($id == 0 || $user_id == 0)
    {
        ecs_header("Location: thanh-vien?act=booking_list\n");
        exit;
    }
...
elseif ($action == 'act_account')
{
    ...
    $surplus = array(
            ...
            'amount'       => $amount,
            'yunqi_paymethod'=>isset($_POST['yunqi_paymethod'])?$_POST['yunqi_paymethod']:'bank',
    );
    ...
    if ($surplus['process_type'] == 1)
    {
        ...
        if ($surplus['rec_id'] > 0)
        {
            ...
            show_message($content, $_LANG['back_account_log'], 'thanh-vien?act=account_log', 'info');
        }
        else
        {
...
elseif ($action == 'cancel')
{
    ...
    if ($id == 0 || $user_id == 0)
    {
        ecs_header("Location: thanh-vien?act=account_log\n");
        exit;
    }
...
elseif ($action == 'pay')
{
    ...
    if ($payment_id == 0)
    {
        ecs_header("Location: thanh-vien?act=account_deposit&id=".$surplus_id."\n");
        exit;
    }
...
elseif ($action == 'del_msg')
{
    ...
    ecs_header("Location: thanh-vien?act=message_list&order_id=$order_id\n");
    exit;
}
...
elseif ($action == 'del_cmt')
{
    ...
    ecs_header("Location: thanh-vien?act=comment_list\n");
    exit;
}
...
elseif ($action == 'merge_order')
{
    ...
    {
        show_message($_LANG['merge_order_success'],$_LANG['order_list_lnk'],'thanh-vien?act=order_list', 'info');
    }
    else
    {
...
elseif ($action == 'act_edit_surplus')
{
    ...
    if ($surplus <= 0)
    {
        ...
        $err->show($_LANG['order_detail'], 'thanh-vien?act=order_detail&order_id=' . $order_id);
    }
    ...
    /* 跳转 */
    ecs_header('Location: thanh-vien?act=order_detail&order_id=' . $order_id . "\n");
    exit;
}
...
elseif ($action == 'save_order_address')
{
    ...
    if (save_order_address($address, $user_id))
    {
        ...
        if ($order_sn) {
            $sql = "UPDATE " . $ecs->table('order_info') .
           " SET lastmodify=".time()."  WHERE order_id = ".$address['order_id'];
            $db->query($sql);
            // include_once(ROOT_PATH . 'includes/cls_matrix.php');
            // $matrix = new matrix;
            // $matrix->createOrder($order_sn);
        }
        ecs_header('Location: thanh-vien?act=order_detail&order_id=' .$address['order_id']. "\n");
        exit;
    }
    else
    {
...
elseif ($action == 'affiliate')
{
    ...
    if(empty($goodsid))
    {
        ...
        $url_format = "thanh-vien?act=affiliate&page=";
...
elseif ($action == 'act_transform_points')
{
    ...
    {
        show_message($_LANG['invalid_points'], $_LANG['transform_points'], 'thanh-vien?act=transform_points');
    }
    ...
    {
        show_message($_LANG['overflow_points'], $_LANG['transform_points'], 'thanh-vien?act=transform_points' );
    }
    ...
    switch ($rule_key)
    {
        case TO_P :
            ...
            show_message(sprintf($_LANG['to_pay_points'],  $num, $points_name[$bbs_key]['title'], $result_points), $_LANG['transform_points'], 'thanh-vien?act=transform_points');

        case TO_R :
            $result_points = floor($num * $to / $from);
            $user->set_points($row['user_name'], array($bbs_key=>0 - $num)); //调整论坛积分
            log_account_change($row['user_id'], 0, 0, $result_points, 0, $_LANG['transform_points'], ACT_OTHER);
            show_message(sprintf($_LANG['to_rank_points'], $num, $points_name[$bbs_key]['title'], $result_points), $_LANG['transform_points'], 'thanh-vien?act=transform_points');
        ...
        case FROM_P :
            ...
            show_message(sprintf($_LANG['from_pay_points'], $num, $result_points,  $points_name[$bbs_key]['title']), $_LANG['transform_points'], 'thanh-vien?act=transform_points');

        case FROM_R :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0-$num, 0, $_LANG['transform_points'], ACT_OTHER); //调整商城积分
            $user->set_points($row['user_name'], array($bbs_key=>$result_points)); //调整论坛积分
            show_message(sprintf($_LANG['from_rank_points'], $num, $result_points, $points_name[$bbs_key]['title']), $_LANG['transform_points'], 'thanh-vien?act=transform_points');
    }
}
elseif ($action == 'act_transform_ucenter_points')
{
    ...
    if ($ratio == 0)
    {
        show_message($_LANG['exchange_deny'], $_LANG['transform_points'], 'thanh-vien?act=transform_points');
    }
    ...
    if ($result === true)
    {
        ...
        $db->query($sql);
        $sql = "INSERT INTO " . $ecs->table('account_log') . "(user_id, {$shop_points[$fromcredits]}, change_time, change_desc, change_type)" . " VALUES ('{$row['user_id']}', '-$exchange_amount', '". time() ."', '" . $cfg['uc_lang']['exchange'] . "', '98')";
        $db->query($sql);
        show_message(sprintf($_LANG['exchange_success'], $exchange_amount, $_LANG['exchange_points'][$fromcredits], $netamount, $credit['title']), $_LANG['transform_points'], 'thanh-vien?act=transform_points');
    }
    else
    {
        show_message($_LANG['exchange_error_1'], $_LANG['transform_points'], 'thanh-vien?act=transform_points');
    }
}
/* 清除商品浏览历史 */
...
elseif ($action == 'delivery_info'){
    ...
    if (empty($order_sn))
    {
        show_message($_LANG['invalid_order_sn'], '', 'thanh-vien?act=order_list');
    }
...
//    $ip = $_SERVER["REMOTE_ADDR"];
    ...
    if ($_POST['sms_code'] != '' && $_POST['username'] != '')
    {
        ...
        if($sms_code == $_SESSION[$username.'-sms_code']){
            // 短信验证码十分钟内有效
            if (time() - $_SESSION[$username.'-send_time'] > 600) {
                show_message('短信验证码已过期', '返回上一页', 'thanh-vien?act=sms_get_password', 'info');
            }
            $user_info = $user->get_profile_by_name($username);
            // 判断会员是否存在
            if (!$user_info['user_id']) {
                show_message(sprintf($_LANG['username_inexistent'], $username), $_LANG['back_page_up'], 'thanh-vien?act=sms_get_password', 'info');
            }
            ...
            show_message('短信验证码错误', '返回上一页', 'thanh-vien?act=sms_get_password', 'info');
        }
    }
    else
    {
