...
if (!$smarty->is_cached($tpl, $cache_id))
{
        ...
        if (!empty($_GET['intro_type']))
        {
            ...
            if ($_GET['intro_type'] == 'is_best' || $_GET['intro_type'] == 'is_new' || $_GET['intro_type'] == 'is_hot' || $_GET['intro_type'] == 'is_promote' || $_GET['intro_type'] == 'is_random')
            {
                ...
                else
                {
                    ...
                    $sql .= " AND " . $_GET['intro_type'] . " = 1 ORDER BY goods_id DESC";
                }
            }
        }
    }
    elseif ($type == 'collection')
    {
        ...
        $sql = "SELECT g.goods_id, g.goods_name,g.goods_nummber ,g.is_hot, g.market_price, g.goods_thumb, IF(g.is_promote = 1 AND '$time' >= g.promote_start_date AND ".
    ...
    while ($goods = $db->fetchRow($res))
    {
        ...
        $goods['goods_nummber'] = $goods['goods_nummber'];
        $goods_list[] = $goods;
    }
