...
//$cate = $db->getRow("SELECT cat_name, cat_id FROM ".$ecs->table('category')." WHERE cat_id = (SELECT id FROM " . $ecs->table('slug') ." WHERE module='category' AND slug = '".$cat_slug."')");

$goods_array = array();
$goods_id  = $db->getOne("SELECT id FROM " . $ecs->table('slug') ." WHERE module='goods' AND slug = '".$slug."'");
//$cat_id  = $db->getOne("SELECT cat_id FROM " . $ecs->table('goods') ." WHERE  goods_id = '".$goods_id."'");
$cate = $db->getRow("SELECT cat_name, cat_id FROM ".$ecs->table('category')." WHERE cat_id = (SELECT cat_id FROM " . $ecs->table('goods') ." WHERE  goods_id = '".$goods_id."')");
$goods_id_vs  = $db->getOne("SELECT id FROM " . $ecs->table('slug') ." WHERE module='goods' AND slug = '".$slug2."'");

$smarty->assign('goods_id', $goods_id);

if($goods_id > 0){
    $goods_array[] = intval($goods_id);
}
if($goods_id_vs > 0){
    array_push($goods_array,intval($goods_id_vs));
}

/* Doi chieu kieu thuoc tinh cua 2 sp */
if(count($goods_array) > 1){
    $where = db_create_in($goods_array, 'goods_id');
    $sql = 'SELECT goods_name, goods_type FROM '.$ecs->table('goods')." WHERE $where";
    $ginfo = $db->getAll($sql);

    if($ginfo[0]['goods_type'] != $ginfo[1]['goods_type']){
        show_message('Sản phẩm bạn chọn để so sánh không cùng loại với nhau');
    }
    $meta_title = 'So sánh '.$ginfo[0]['goods_name'].' và '.$ginfo[1]['goods_name'];
    $meta_desc = 'So sánh chi tiết về giá, khuyến mãi, cấu hình, ưu nhược điểm giữa '.$ginfo[0]['goods_name'].' và '.$ginfo[1]['goods_name'].' | '.$_CFG['shop_name'];
}


if (!empty($goods_array) && count($goods_array) > 1)
{

    foreach ($goods_array as $key=>$val)
    {
       $goods_array[$key]=intval($val);
    }

    $where = db_create_in($goods_array, 'id_value');
    ...
    $where = db_create_in($goods_array, 'g.goods_id');
...
$smarty->assign('page_title', $meta_title.' | '.$_CFG['shop_name']);
$smarty->assign('description', $meta_desc);
$smarty->assign('ur_here',    $position['ur_here']);
$smarty->assign('cate',    $cate);
$smarty->assign('compare_title', $meta_title);
