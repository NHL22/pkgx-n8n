...
if (empty($brand_id))
{
    ...
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
        ...
        $smarty->assign('brand_list', getBrandsList());
    }
...
if (!$smarty->is_cached('brand.dwt', $cache_id))
{
    ...
    if(isset($cate_info['cat_name'])){
        $page_title = $cate_info['cat_name'].' '.$brand_info['brand_name'];
    ...
    $smarty->assign('long_desc', htmlspecialchars($long_desc));
    $smarty->assign('cat_desc', htmlspecialchars($cat_desc));

    /* 赋值固定内容 */
    assign_template();
    $position = assign_ur_here($cate, $page_title);
    $smarty->assign('page_title',    $page_title.' - '.$_CFG['shop_name']);
    $smarty->assign('ur_here',        $position['ur_here']); // 当前位置
...
{
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_long_desc, b.brand_cat_desc, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
...
function brand_recommend_goods($type, $brand, $cat = 0)
{
    ...
    if ($result === NULL)
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.deal_price, g.partner_price,g.goods_number, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
