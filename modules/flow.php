...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    {
        /*nobj fix a.attr_type != 0 */
        $sql = "SELECT a.attr_id, a.attr_name, a.attr_type, ".
            "g.goods_attr_id, g.attr_value, g.attr_price " .
        'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
        'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
        "WHERE a.attr_type IN (1,2)  AND g.goods_id = '" . $goods->goods_id . "' " .
    ...
    /* fix for  installment */
    if(isset($goods->installment) && $goods->installment == 1){
        $_CFG['one_step_buy'] = 1;
        $_SESSION['direct_shopping'] = 1;
        $result['link_installment'] = build_uri('goods', array('gid'=> $goods->goods_id));
    }
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (!check_consignee_info($consignee, $flow_type))
    {
        ecs_header("Location: gio-hang?step=consignee\n");
        //exit;
        //show_message('Thông tin chưa hợp lệ', 'Trở lại', 'gio-hang?step=consignee', 'error');
    }

    /* validate email va mobile */
    if(!is_email($consignee['email'])){
        show_message('Email không hợp lệ', 'Trở lại', 'gio-hang?step=consignee', 'error');
    }
    if(!is_tel($consignee['tel'])){
        show_message('Số điên thoại không hợp lệ', 'Trở lại', 'gio-hang?step=consignee', 'error');
    }

    $_SESSION['flow_consignee'] = $consignee;
    /* add by Nobj. Nếu là Virtual Goods List thì ko cần nhập tỉnh thành */
    if(!empty($consignee['province'])){
        ...
        $consignee['address'] = $consignee['address'].", $city, $province";
    }
    ...
    foreach ($shipping_list AS $key => $val)
    {
        ...
        $shipping_list[$key]['insure_formated']     = strpos($val['insure'], '%') === false ?
            price_format($val['insure']) : $val['insure'];
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
        ...
        $smarty->assign('total_bonus',    price_format(get_total_bonus()));
        ...
        {
            $result['cod_fee'] = price_format($result['cod_fee']);
        }
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
        ...
        'card_message'    => filter_var(trim($_POST['card_message']), FILTER_SANITIZE_STRING),
        ...
        'postscript'      => filter_var(trim($_POST['postscript']), FILTER_SANITIZE_STRING),
 ...
 */
elseif ($_REQUEST['step'] == 'quick_order'){
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
    {
        ...
        flow_cart_stock($_cart_goods_stock) ;
        //设置(cart_goods_stock, _cart_goods_stock 美元) ;
    }
...
elseif ($_REQUEST['step'] == 'drop_goods')
{
    ...
    /** add nobj  */
    if(isset($_GET['links'])){
        $url = filter_var($_GET['links'], FILTER_SANITIZE_URL);
