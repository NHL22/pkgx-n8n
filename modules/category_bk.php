...
setcookie('ECS[display]', $display, time() + 86400 * 7, NULL, NULL, NULL, TRUE);
/*------------------------------------------------------ */
//-- PROCESSOR
/*------------------------------------------------------ */
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if(isset($_GET['fb_comment_id']) || isset($_GET['client']) || isset($_REQUEST['page']) || isset($_REQUEST['price_min']) || isset($_REQUEST['price_max']) || isset($_REQUEST['rc']) || isset($_REQUEST['filter_attr']) || isset($_REQUEST['display']) || isset($_REQUEST['sort']) || isset($_REQUEST['order']))
    {
    ...
    if ($cat['grade'] > 1)
    {
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$temp_key]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str,'rc'=>$recommend), $cat['cat_name']);
        ...
        $price_grade[0]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str,'rc'=>$recommend), $cat['cat_name']);
    ...
    foreach ($brands AS $key => $val)
    {
        ...
        $brands[$temp_key]['url'] = build_uri('category', array('cid' => $cat_id_in, 'bid' => $val['brand_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str,'rc'=>$recommend), $cat['cat_name']);
    ...
    $brands[0]['url'] = build_uri('category', array('cid' => $cat_id_in, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str,'rc'=>$recommend), $cat['cat_name']);
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, custom_name, meta_desc, meta_title, meta_robots, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.seller_note, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
